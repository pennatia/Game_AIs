...
sys.setrecursionlimit(2500)#Add limit for recursion
from AI_Checks import checkWin, minimax
from AI_Checks import Goal_Based_Agent
...
def AI(aiSkill,place,turn,AIturn):
    ...
    if aiSkill == 1: #Easy
        if place[4] == 0:
            ...
            midRightPress()
    if aiSkill == 2: #Medium
        ## This is where the Goal-Based agent is implemented ##
        F = Goal_Based_Agent(place)
        print(F)
        if F != None:
            if F == 0:
                topLeftPress()
